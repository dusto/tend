...
var Methods = []Method{
	...
	{Name: "session.set_mode", Direction: PluginToDaemon, Params: SessionSetModeParams{}, Result: SessionSetModeResult{}, Summary: "Set a session's active mode (reasoning/thought level) from its available modes; emits agent_mode_updated. Errors when the provider offers no modes."},
