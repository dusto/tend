...
var Methods = []Method{
	...
	{Name: "mcp.resolve", Direction: PluginToDaemon, Params: MCPResolveParams{}, Result: MCPResolveResult{}, Summary: "Resolve an MCP editor-tools bridge token (from a session's mcpServers declaration) to the session id the bridge serves; errors on an unknown token or an ended session."},
	{Name: "session.attach", Direction: PluginToDaemon, Params: SessionAttachParams{}, Result: SessionAttachResult{}, Summary: "Make the calling client follow a session: its clarification prompts and waiting_* status are delivered to attached clients. Does not claim the editor binding, and does NOT gate approvals — approval_requested/approval_resolved broadcast to every client on the workspace stream, independent of attach. Until any client attaches, a session's clarification prompts broadcast to all clients."},
	...
	{Name: "file.open", Direction: PluginToDaemon, Params: FileOpenParams{}, Result: FileOpenResult{}, Summary: "Open a repo file in the session's bound editor for the user to see (non-mutating, worktree-bounded); a no-op for a headless session (open=false)."},
	{Name: "file.patch", Direction: PluginToDaemon, Params: FilePatchParams{}, Result: FileMutationResult{}, Summary: "Apply non-overlapping text edits to a repo file (approval-gated, base-checked; task association optional); single-target change set."},
	{Name: "file.write", Direction: PluginToDaemon, Params: FileWriteParams{}, Result: FileMutationResult{}, Summary: "Replace a repo file's whole content (approval-gated, base-checked; task association optional); single-target change set."},
	...
	{Name: "file.diff", Direction: PluginToDaemon, Params: FileDiffParams{}, Result: FileDiffResult{}, Summary: "Fetch a change set's captured before/after snapshots for review (read-only)."},
	...
	{Name: "pane.run", Direction: PluginToDaemon, Params: PaneRunParams{}, Result: PaneRunResult{}, Summary: "Run a command in a pane (approval-gated; task association optional); output arrives on the pane stream."},
	...
	{Name: "prompt.raise", Direction: DaemonToClient, Params: PromptRaiseParams{}, Result: nil, Summary: "Raise a clarification prompt and waiting_* status to a session's attached clients. Approvals are NOT raised here — they broadcast on the workspace stream as approval_requested (see session.attach)."},
