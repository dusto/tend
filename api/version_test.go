...
func TestVersionsSatisfies(t *testing.T) {
	cur := CurrentVersions() // plugin_to_daemon 0.10.0, daemon_to_editor 0.3.0
	cases := []struct {
		name string
		req  Versions
		ok   bool
		...
		{"equal current", Versions{PluginToDaemon: "0.10.0"}, true},
		{"higher minor required", Versions{PluginToDaemon: "0.11.0"}, false},
		{"higher patch required", Versions{PluginToDaemon: "0.10.1"}, false},
