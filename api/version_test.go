...
func TestVersionsSatisfies(t *testing.T) {
	cur := CurrentVersions() // plugin_to_daemon 0.8.0, daemon_to_editor 0.3.0
	cases := []struct {
		name string
		req  Versions
		ok   bool
		...
		{"lower minor required", Versions{PluginToDaemon: "0.7.0"}, true},
		{"equal current", Versions{PluginToDaemon: "0.8.0"}, true},
		{"higher minor required", Versions{PluginToDaemon: "0.9.0"}, false},
		{"higher patch required", Versions{PluginToDaemon: "0.8.1"}, false},
