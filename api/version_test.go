...
func TestVersionsSatisfies(t *testing.T) {
	cur := CurrentVersions() // plugin_to_daemon 0.6.0, daemon_to_editor 0.2.0
	cases := []struct {
		name string
		req  Versions
		ok   bool
		...
		{"lower minor required", Versions{PluginToDaemon: "0.5.0"}, true},
		{"equal current", Versions{PluginToDaemon: "0.6.0"}, true},
		{"higher minor required", Versions{PluginToDaemon: "0.7.0"}, false},
		{"higher patch required", Versions{PluginToDaemon: "0.6.1"}, false},
