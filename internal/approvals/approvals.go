...
// Pending is a snapshot of an approval still awaiting a decision. Approvals have
// no deadline: they persist until answered or their turn is cancelled (see the
// no-TTL ADR), so Created is the only timestamp.
type Pending struct {
	ID        api.ApprovalID
...
// otherwise. On ctx cancellation the pending approval is evicted (and its
// eviction signalled to clients) and ctx.Err is returned; the caller owns the
// session's status in that case.
func (g *Gate) Request(ctx context.Context, sess *session.Session, kind string, detail json.RawMessage) (Outcome, error) {
	...
	case <-ctx.Done():
		...
		if g.claim(id) != nil {
			// The turn died before the approval was answered (provider crash,
			// agent.cancel, or session end — all reach here as ctx cancellation).
			// The pending is now unanswerable, so signal its eviction to clients:
			// a subscribed UI clears the stale approval live instead of waiting for
			// the next approval.list sync. Approved=false with the cause in Reason;
			// this is not a user denial. The session's status is owned by whoever
			// cancelled the turn, so we do not touch it here.
			g.emit2(api.WorkspaceStream(sess.WorkspaceID), "approval_resolved", api.ApprovalResolved{
				SessionID:  sess.ID,
				ApprovalID: id,
				Approved:   false,
				Reason:     "cancelled: turn ended before the approval was answered",
			})
