...
func TestSmokeAgentTurnStreamsEvents(t *testing.T) {
	...
	// The turn opens with the user_prompt (the request we sent) then
	// agent_prompt_usage (its measured size), before the agent's streamed output.
	want := []string{"user_prompt", "agent_prompt_usage", "agent_message_chunk", "agent_message_chunk", "tool_call", "tool_call_update", "turn_end"}
