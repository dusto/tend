...
func TestReplayReconnectDedup(t *testing.T) {
	...
	// The first connection processed seqs 1..7 (user_prompt then agent_prompt_usage
	// open the turn).
	dd := events.NewDeduper()
	got := first.Events(stream)
	if len(got) != 7 {
		t.Fatalf("first delivery = %d events, want 7", len(got))
	}
	for _, ev := range got {
		if !dd.Fresh(ev) {
	...
	// daemon replays (2, tail] = 3..7, which the Deduper drops as duplicates.
	...
	if !reconnect.WaitEventCount(stream, 5, 3*time.Second) {
...
func TestMultipleSessionsIndependent(t *testing.T) {
	...
	if !c.WaitEventCount(a.StreamID, 7, 3*time.Second) || !c.WaitEventCount(b.StreamID, 7, 3*time.Second) {
		t.Fatalf("turns incomplete: a=%v b=%v", c.EventTypes(a.StreamID), c.EventTypes(b.StreamID))
	}
	// No leakage: each stream carries exactly its own turn (7 records), and every
	// record's stream id matches.
	for _, s := range []api.StreamID{a.StreamID, b.StreamID} {
		evs := c.Events(s)
		if len(evs) != 7 {
			t.Errorf("stream %s = %d events, want 7 (leakage or loss)", s, len(evs))
		}
		for _, ev := range evs {
			if ev.StreamID != s {
	...
	if !rc.WaitEventCount(a.StreamID, 5, 3*time.Second) || !rc.WaitEventCount(b.StreamID, 7, 3*time.Second) {
		t.Fatalf("reconnect replay incomplete: a=%v b=%v", rc.EventTypes(a.StreamID), rc.EventTypes(b.StreamID))
	}
	// Independent cursors: a replays only (2, tail] = 3..7; b replays the whole 1..7.
	for _, ev := range rc.Events(a.StreamID) {
		if ev.Seq < 3 {
			t.Errorf("stream a replayed seq %d, want only (2, tail]", ev.Seq)
		}
	}
	if len(rc.Events(b.StreamID)) != 7 {
		t.Errorf("stream b replay = %d, want 7", len(rc.Events(b.StreamID)))
	}
